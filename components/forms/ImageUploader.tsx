// FINAL & CORRECT - components/forms/ImageUploader.tsx
'use client';
import { useState } from 'react';

// This interface defines the function the parent component will pass down.
interface ImageUploaderProps {
  onImageSelected: (file: File | null, previewUrl: string | null) => void;
}

// Ensure the component is exported with this exact name.
export function ImageUploader({ onImageSelected }: ImageUploaderProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setPreview(result);
        onImageSelected(file, result); // Send data back to the parent
      };
  ...
  return (
    <div>
      <label htmlFor="image-upload" className="block text-lg font-semibold text-gray-800 mb-2">Upload Room Image</label>
